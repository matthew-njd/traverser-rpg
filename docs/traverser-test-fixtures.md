...
## 11. Health Derivation (T3) — verified 2026-07-26

The eight cases T3 §10 owes, generated by executing the T3 §5/§8 algorithms in Node rather than transcribing prose. All local times are **America/New_York (UTC−4, EDT)** and the offset is part of the fixture — session IDs are epoch-derived. Tier XP rates (GDD 1 §2.2): Tier 1 = 3/min, Tier 2 = 5/min, Tier 3 = 7/min capped at 20 cumulative min/day then 5/min.

### 11.1 Threshold tables — `HRmax = 220 − age`, bounds `ceil(HRmax × pct)`

| Age | HRmax | Tier 1 ≥ (50%) | Tier 2 ≥ (70%) | Tier 3 ≥ (85%) |
|---|---|---|---|---|
| 30 | 190 | 95 | 133 | **162** (ceil 161.5) |
| 55 | 165 | **83** (ceil 82.5) | **116** (ceil 115.5) | **141** (ceil 140.25) |

Three of the five bounds are non-integral before `ceil` — a `round` implementation returns 162→162 but 83→83 vs 82, 141→141 vs 140. Age 55 is the row that pins `ceil`.

### 11.2 Bucketing — mean BPM per whole local minute (age 30 thresholds)

| Samples in minute | Mean | Tier |
|---|---|---|
| 130, 134, 136 | 133.33 | **2** (mean ≥ 133 even though one sample is below) |
| 128, 131 | 129.5 | 1 |
| 131, 133, 135 | 133 (exact bound) | **2** — bounds are inclusive |
| 90, 98 | 94 | **0** — untiered, one below t1 = 95 |
| *(none)* | — | **0** — a minute with no samples contributes nothing; sparse sampling never invents minutes |

### 11.3 Segmentation — 9-minute gap does not close (one session)

Timeline: Tier 1 minutes 10:00–10:14 (15 min) · no samples 10:15–10:23 (9 min) · Tier 1 minutes 10:24–10:33 (10 min), 2026-07-19.

**One session**: `started_at` 10:00, `ended_at` 10:33, **tier1_minutes = 25** — the 9 gap minutes are inside the session but contribute to no tier.

**Boundary row:** the same timeline with an exactly-**10**-minute gap (10:15–10:24) is still **one session** (26 min span, 25 tier-1 minutes). GDD 11 §8.1 closes a session only after **more than** 10 consecutive sub-Tier-1 minutes — 10 exactly does not close; 11 does. This row pins the strict inequality.

### 11.4 Segmentation — 11-minute gap closes (two sessions, two IDs)

Timeline: Tier 1 10:00–10:14 · no samples 10:15–10:25 (11 min) · Tier 1 10:26–10:35, 2026-07-19 EDT.

| Session | `started_at` (local) | `ended_at` | tier1_min | `external_session_id` |
|---|---|---|---|---|
| 1 | 10:00 | 10:14 | 15 | `hr:1784469600` |
| 2 | 10:26 | 10:35 | 10 | `hr:1784471160` |

Session 1's `ended_at` is 10:14 — the last Tier 1+ minute, **not** the end of the gap.

### 11.5 Midnight-crossing session — one session, two day rollups

Tier 1 minutes 23:50 (Jul 19) through 00:15 (Jul 20), contiguous — 26 minutes.

**One session**: `started_at` 2026-07-19T23:50 EDT, `ended_at` 2026-07-20T00:15, tier1_minutes = 26, id `hr:1784519400`. Day rollups: `activity_day` **2026-07-19 gets 10** tier-1 minutes, **2026-07-20 gets 16**. The session is never split; only the per-date `sync_delta` minutes are — the overactivity 90-minute rule reads the session, not the days.

### 11.6 Tier 3 daily cap — client output vs. server charge are different numbers

Client ships raw uncapped Tier 3 minutes (T3 §5.5); the server charges against the day's post-merge cumulative (T2 §4 step 4).

| Event | Client mints (`hr_tier: 3`) | Day's t3 before merge | Charged at 7 XP | Charged at 5 XP | XP |
|---|---|---|---|---|---|
| First sync, 15 Peak min | `minutes_delta: 15` | 0 | 15 | 0 | **105** |
| Later delta, 12 more Peak min | `minutes_delta: 12` | 15 | 5 | 7 | **70** |

The second row is T2 §4's anchor case. A client that pre-capped would mint 5 in row 2 and the server would charge 5 at Peak — same XP *that day*, but the day's stored `tier3_minutes` would read 20 instead of 27 and the under-report is permanent. A server that evaluated the cap per-delta would charge all 12 at 7 XP (84) — over by 14, silently, in the player's favour.

### 11.7 Step high-water mark — rising totals, then a downward revision

Four successive reads of the same `activity_date`:

| Read | Observed total | Delta minted | Mark after |
|---|---|---|---|
| 1 | 2,000 | **2,000** | 2,000 |
| 2 | 5,400 | **3,400** | 5,400 |
| 3 | 5,100 *(downward revision)* | **none** | **5,400 — not lowered** |
| 4 | 5,600 | **200** | 5,600 |

Total minted across all reads = 5,600 = the final observed total. Read 4 is the trap: after a downward revision the delta is against the *unlowered* mark (5,600 − 5,400 = 200), never against the revised observation (5,600 − 5,100 = 500 would re-send 300 already-reported steps).

### 11.8 Session merge via backfill — the earlier ID wins

Before backfill: the two sessions of §11.4 (`hr:1784469600`, `hr:1784471160`). A later read backfills Tier 1 minutes at 10:16–10:24, cutting every gap below the close threshold.

**After backfill: one session** — `started_at` 10:00, `ended_at` 10:35, tier1_minutes = **34** (15 + 9 + 10; the two remaining sample-less minutes 10:15 and 10:25 are in-session, untiered). Surviving id: **`hr:1784469600`** — the earlier. `hr:1784471160` is tombstoned locally and never sent again; the server's orphaned row is inert. The merged session's next upsert **sets** its totals (T2 §6.3) — nothing is added.
